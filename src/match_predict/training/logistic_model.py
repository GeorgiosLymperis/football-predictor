...
def impute(X: pd.DataFrame, means: pd.Series) -> pd.DataFrame:
    return X.fillna(means)


def hda_probs(model: LogisticRegression, scaler: StandardScaler, encoder, X: pd.DataFrame,
...
class LogisticPyfuncModel(mlflow.pyfunc.PythonModel):

    def load_context(self, context):
        data = np.load(context.artifacts['logistic_params'], allow_pickle=True)
    ...
    def predict(self, context, model_input: pd.DataFrame) -> pd.DataFrame:
        from match_predict.predict.logistic import predict_outcome_probs
        rows = []
