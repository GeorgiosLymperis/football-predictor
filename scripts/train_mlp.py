...
from match_predict.training.mlp_model import MLPPyfuncModel, hda_probs

MODEL_PREFIX = 'mlp'


def _make_fns(features: list[str], mlp_params: dict):
    def fit_fn(train_df):
...
def select_variant(sl_features: pd.DataFrame, cfg: dict, backtest_cfg: dict) -> str:
    ...
    for variant in cfg['feature_variants']:
        fit_fn, predict_fn = _make_fns(FEATURE_VARIANTS[variant], cfg['mlp_params'])
...
def run_backtest(sl_features: pd.DataFrame, cfg: dict, backtest_cfg: dict):
    ...
    fit_fn, predict_fn = _make_fns(features, cfg['mlp_params'])
...
def main():
    ...
    mlp_params_out = dict(
        coefs=np.array(model.coefs_, dtype=object),
    ...
    params_path = tmp_dir / 'mlp_params.npz'
    state_path = tmp_dir / 'team_state.npz'
    np.savez_compressed(params_path, **mlp_params_out)
    ...
    version = registry.log_run(
        model_name,
        params={
            'hidden_layer_sizes': str(cfg['mlp_params']['hidden_layer_sizes']),
            'alpha': cfg['mlp_params']['alpha'], 'feature_variant': best_variant, 'league': args.league,
        },
        metrics={'rps': summary['rps'], 'baseline_rps': summary['baseline_rps']},
        tags={'data_hash': str(pd.util.hash_pandas_object(df).sum())},
        python_model=MLPPyfuncModel(),
        artifacts={'mlp_params': str(params_path), 'team_state': str(state_path)},
    ...
    if registry.promote_if_better(model_name, version, summary['rps'], summary['baseline_rps'],
        ...
        out_dir = registry.export_champion(
            ...
            {'mlp_params.npz': str(params_path), 'team_state.npz': str(state_path)},
            out_dir=registry.MODELS_DIR / args.league / 'mlp',
