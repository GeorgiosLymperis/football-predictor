...
from match_predict.training.xgb_model import EloXGBPyfuncModel, hda_xgb_probs

def _make_fns(features: list[str], xgb_params: dict):
    def fit_fn(train_df):
        encoder = LabelEncoder()
        y = encoder.fit_transform(train_df['result'])
        weights = compute_sample_weight(class_weight='balanced', y=y)
        model = xgb.XGBClassifier(**xgb_params)
        model.fit(train_df[features], y, sample_weight=weights)
        return model, encoder

    def predict_fn(fitted, eval_df):
        model, encoder = fitted
        return hda_xgb_probs(model, encoder, eval_df[features])
...
def run_backtest(sl_features: pd.DataFrame, cfg: dict, backtest_cfg: dict):
    ...
    fit_fn, predict_fn = _make_fns(features, cfg['xgb_params'])
...
def main():
    ...
    version = registry.log_run(
        model_name,
        params={**cfg['xgb_params'], 'feature_variant': best_variant, 'league': args.league},
        metrics={'rps': summary['rps'], 'baseline_rps': summary['baseline_rps']},
        tags={'data_hash': str(pd.util.hash_pandas_object(df).sum())},
        python_model=EloXGBPyfuncModel(),
        artifacts={'xgb_model': str(model_path), 'team_state': str(state_path)},
    ...
    if registry.promote_if_better(model_name, version, summary['rps'], summary['baseline_rps'],
        ...
        out_dir = registry.export_champion(
            ...
            {'xgb_model.ubj': str(model_path), 'team_state.npz': str(state_path)},
            out_dir=registry.MODELS_DIR / args.league / 'elo_xgb',
